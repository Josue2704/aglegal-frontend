...
export function TaskForm({ caseId, onDone }: { caseId?: number; onDone: () => void }) {
  ...
  function submit() {
    ...
    create.mutate({
      ...
      es_critico: critico,
      monto_adicional: monto ? Number(monto) : undefined,
    })
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <label className={LBL}>Monto adicional ($, opcional)</label>
          <Input type="number" step="0.01" min="0" className="h-8 text-sm" placeholder="0.00" value={monto} onChange={(e) => setMonto(e.target.value)} />
        </div>
        <div className="space-y-1">
      ...
      {Number(monto) > 0 && (
        <p className="text-[11px] text-amber-600">
          Sumará {formatCurrency(Number(monto))} a los honorarios contratados del expediente (queda en la bitácora).
        </p>
      )}
      ...
      <div className="flex items-center gap-2.5 px-3 py-2.5 group">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded((v) => !v)}>
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap text-[10px] text-muted-foreground">
            ...
            {task.monto_adicional > 0 && <span className="text-amber-600">+{formatCurrency(task.monto_adicional)}</span>}
