...
export const clientsApi = {
  list: (search?: string, archived?: boolean) =>
    api.get<Client[]>('/clients', { params: { search: search || undefined, archived: archived || undefined } }).then((r) => r.data),
  ...
  restore: (id: number) => api.post<Client>(`/clients/${id}/restore`).then((r) => r.data),
  purge: (id: number) => api.delete(`/clients/${id}/purge`),
